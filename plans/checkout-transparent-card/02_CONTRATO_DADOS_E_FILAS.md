...
| Pagar.me | `secret_key` privada e, no método `card_credit`, `public_key`, máximo de parcelas e valor mínimo por parcela. |
...
No Pagar.me, `card_credit` recebe a `public_key` e a política de parcelas da integração; a `secret_key` já armazenada na integração é copiada para a chave cifrada do método. O checkout expõe somente as opções dentro desse limite e a tentativa valida novamente o limite e o valor mínimo. O token do cartão é criado no navegador e expira no provedor; o Edge encaminha apenas esse token para a tentativa, junto do endereço de cobrança necessário ao pedido.
