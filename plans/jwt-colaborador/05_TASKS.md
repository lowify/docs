...
**Ajuste identificado no teste integrado (2026-08-27):** a whitelist local avaliava apenas o basename do script e, por isso, negava `api/dashboard/sales/list.php` antes da chamada ao Gateway. A regra agora usa o caminho relativo para endpoints sob `api/` e mapeia esse endpoint existente para `sales`; páginas continuam usando o basename e todas as demais rotas locais permanecem deny-by-default.
